...
public class Time {
	...
	public static boolean verifyString(String string) {
		...
		for (int i = 0; i < string.length(); i++) {
			...
			try {
				...
				numeroMemory = numeroMemory + String.valueOf(numero);
			} catch (Exception e) {
				if (i == 0)return false;
				if (letra != 'd' || letra != 'h' || letra != 'm' || letra != 's')return false;
				if (numeroMemory == "") return false;
				numeroMemory = "";
			}
		}
		return true;
	...
	public static String getFormattedStringTimeOfSeconds(int time) {
		...
		String format = ""; // Formato %days% days %hours% hours %minutes% minutes %seconds% seconds / 1
							// days 4 hours 30 minutes 2 seconds
